...
- **Purpose**: 帮助用户养成每日记录反思的习惯，借助AI能力降低写作门槛，同时将AI润色后的优质内容便捷同步到用户已有的"一本日记"App中。
...
- **FR-9**: 面包屑导航 — 在`BREADCRUMB_SECTIONS`中新增"三省"项，label为"三省"
...
- **Given**: 用户已登录，且在首页
- **When**: 页面渲染完成
- **Then**: 在纪念日卡片之后显示"三省"日记卡片，卡片顶部有📝 三省标题，下方有快捷输入框+添加按钮，列表区域展示最近6条日记（日期+内容前30字），底部有"查看全部 (N) →"按钮
...
- **Given**: 任意页面
- **When**: 查看面包屑导航
- **Then**: "三省"出现在面包屑中，位于"待办清单"之后、"纪念日"之前（或按设计顺序）
- **Verification**: `human-judgment`
